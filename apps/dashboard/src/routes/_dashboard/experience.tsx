...
import { Plus, MoreHorizontal, Pencil, Trash2, Building2, MapPin, Calendar, Loader2, Briefcase } from 'lucide-react'
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2" onClick={() => setEditingExperience(null)}>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="p-6">
            <p className="text-destructive">Failed to load experiences. Please try again.</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-muted-foreground">No work experience yet. Add your first position!</p>
          </CardContent>
        </Card>
      )}
    </div>
...
function ExperienceCard({ experience, index, onEdit }: { experience: Experience; index: number; onEdit: (exp: Experience) => void }) {
  ...
  const technologies = experience.techStack?.split(',').map(t => t.trim()).filter(Boolean) || []

  return (
    <Card className={`relative md:ml-16 ${!experience.isActive ? 'opacity-60' : ''}`}>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {experience.isCurrent && <Badge className="bg-emerald-500/10 text-emerald-600">Current</Badge>}
              {!experience.isActive && <Badge variant="secondary">Inactive</Badge>}
            </div>
            <CardDescription className="flex items-center gap-4 flex-wrap">
              <span className="flex items-center gap-1">
                <Building2 className="h-4 w-4" />{experience.company}
              </span>
              <span className="flex items-center gap-1">
...
function ExperienceDialog({ experience, onClose }: { experience: Experience | null; onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && experience) {
        await updateExperience.mutateAsync({ id: experience.id, data: formData })
      } else {
    ...
    <DialogContent className="max-w-2xl">
      <form onSubmit={handleSubmit}>
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing ? 'Update your work experience' : 'Add a new position to your experience timeline'}
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4">
          ...
          <div className="flex items-center space-x-2">
            <Switch 
              ...
              onCheckedChange={(checked) => setFormData({ ...formData, isCurrent: checked, endDate: checked ? null : formData.endDate })}
            />
